...
export default class anidb {
  ...
  private static readonly mutex = new Mutex(); // used to prevent more than 1 request per 2 seconds
  ...
  private async requestShowData(cacheFilePath: string, id: string): Promise<AnimeResponse> {
    const release = await anidb.mutex.acquire(); // wait till a request is possible
